...
export default function ErroMessagem({ onClose }: ErroMessagemPros) {
   ...
   useEffect(() => {
      ...
      window.addEventListener("scroll", handleSchool)
      return () => window.removeEventListener("scroll", handleSchool)
   }, []);
    ...
    return (
        <div className="erro-mensagem" >
            <div className="error" ref={ alertRef }>
                ...
                <div className="error__title">Todos os campos são obrigatórios!</div>
                <button className="ml-3 bg-white text-red-500 px-2 py-1 rounded" onClick={onClose} >✖</button>
            </div>
        </div>
    )
}